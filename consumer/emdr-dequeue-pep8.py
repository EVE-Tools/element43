...
if not dbpass:
    ...
    dbcon = psycopg2.connect("host=" + dbhost + " user=" + dbuser +
...
#connect to memcache
mc = pylibmc.Client(
    [mcserver], binary=True, behaviors={"tcp_nodelay": True, "ketama": True})
...
def thread(message):
    ...
    mc = pylibmc.Client([mcserver], binary=True, behaviors={
                ...
                for order in item_region_list:
                    ...
                    if (order.generated_at > (now_dtime_in_utc() - datetime.timedelta(hours=max_order_age))):
                        ...
                        if (order.type_id != statTypeID) or (order.region_id != statRegionID):
                            ...
                            if result:
                                ...
                                if recordCount is not None:
                                    stddev = result[1]
                                    mean = result[2]
                                if (stddev is not None) and (recordCount > 3):
                        ...
                        if result:
                            ...
                            statsData.append(row)
                            row = (
                                order.order_id, order.type_id, order.station_id, order.solar_system_id,
