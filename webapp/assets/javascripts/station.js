	...
	options = {
		...
		'onSelect': function(value, data){
            ...
            if (id.indexOf('system_') != -1) {
                $('#import').load('/market/trading/station/' + staStationID + '/import/system/' + id.replace('system_', '') + '/', function(){$("[rel=tooltip]").tooltip();});
            } else if (id.indexOf('region_') != -1) {
                $('#import').load('/market/trading/station/' + staStationID + '/import/region/' + id.replace('region_', '') + '/', function(){$("[rel=tooltip]").tooltip();});
            }
        },
	};
