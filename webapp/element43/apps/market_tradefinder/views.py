    ...
    if request.method == 'POST':  # If the form has been submitted...
        ...
        if form.is_valid():
            ...
            for trade in trades:
                ...
                trade['top_buy'] = Orders.active.filter(mapregion=start, invtype_id=trade['id']).order_by('price')[:5]
                trade['top_sell'] = Orders.active.filter(mapregion=destination, invtype_id=trade['id']).order_by('-price')[:5]
